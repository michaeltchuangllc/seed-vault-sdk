...
class PrepopulateKnownAccountsUseCase @Inject constructor(
    ...
    suspend fun populateKnownAccounts(
        ...
        when (purpose) {
            Authorization.Purpose.SIGN_SOLANA_TRANSACTIONS -> {
                populateEd25519Accounts(seed, purpose, BIP44_COIN_TYPE_SOLANA, SOLANA_NUM_KNOWN_ACCOUNTS)
            }
            Authorization.Purpose.SIGN_ALGORAND_TRANSACTIONS -> {
                populateEd25519Accounts(seed, purpose, BIP44_COIN_TYPE_ALGORAND, ALGORAND_NUM_KNOWN_ACCOUNTS)
            }
            Authorization.Purpose.SIGN_BITCOIN_TRANSACTIONS,
            Authorization.Purpose.SIGN_ETHEREUM_TRANSACTIONS -> {
                Log.d(TAG, "Skipping known accounts for $purpose - not yet implemented")
            }
        }
    }

    private suspend fun populateEd25519Accounts(
        seed: Seed,
        purpose: Authorization.Purpose,
        coinType: Int,
        numAccounts: Int
        ...
        val derivationRootPath = Bip32DerivationPath.newBuilder()
            ...
            .appendLevel(BipLevel(coinType, true))
    ...
    companion object {
        ...
        private const val ALGORAND_NUM_KNOWN_ACCOUNTS = 50
    }
}